...
unsigned long getSet(unsigned long adr, CACHE* c){
...
unsigned long getTag (unsigned long adr,CACHE* c){
...
//	printf("args are: memCyc: %d, bsize:%d, l1size:%d l1cyc: %d , l1way:%d\n",_memCyc, _blockSize, l1Size, l1Cyc, l1Ways);
	...
	l1->size = l1Size;
	l1->cyc = l1Cyc;
	l1->waysNum = pow(2,l1Ways);
	l1->waysArr = alloacteWays(l1->waysNum, l1Size,blockSize);
	l1->setsize = Log2(l1->waysArr[0].totalLines);
	zeroStats(&l1->stats);
	...
	l2->size = l2Size;
	l2->cyc = l2Cyc;
	l2->waysNum = pow(2,l2Ways);
	l2->waysArr = alloacteWays(l2->waysNum,l2Size,blockSize);
	l2->setsize = Log2(l2->waysArr[0].totalLines);
	zeroStats(&l2->stats);
...
LINE* findInCache(CACHE* cache, unsigned long set, unsigned long tag ){
...
unsigned long concatenate(unsigned long set, unsigned long tag, int setSize, int Bsize) {
	return (set + (tag *(int)EXP2(setSize))) *(int)EXP2(Bsize);
}

LINE* addToCache(CACHE* c, unsigned long set,unsigned long tag){
...
//				globalStats.cyc+=l2->cyc-l1->cyc;
				//printf("tag %d , set %d \n",evictedLine->tag,set);	
				unsigned long adr = concatenate(set, evictedLine->tag, c->setsize, blockSize);
...
//			}
			...
			unsigned long adr = concatenate(set, c->waysArr[availableWayNum].linesArr[set].tag, c->setsize, blockSize);
...
void updateCache(unsigned long adr, bool isWrite){

	globalStats.totalInsts++;

	unsigned long set1 = getSet(adr,l1);
